...
public class Search {
    ...
    public static Seq<Player> players(String input, EnumSet<SearchOption> options) {
    ...
    public static Seq<Map> maps(String input, EnumSet<SearchOption> options) {
    ...
    public static Seq<Fi> saves(String input, EnumSet<SearchOption> options) {
        int id = parseId(input) - 1;
    ...
    public static Team team(String input, EnumSet<SearchOption> options) {
        int id = parseId(input);

        return Structs.find(Team.all, team -> {
            if (options.contains(SearchOption.USE_ID) && team.id == id)
                return true;

            return options.contains(SearchOption.IGNORE_CASE) ? team.name.equalsIgnoreCase(input) : team.name.equals(input);
        });
    }

    public static <T extends UnlockableContent> T content(String input, ContentType type, EnumSet<SearchOption> options) {
        int id = parseId(input);

        return content.<T>getBy(type).find(content -> {
            if (options.contains(SearchOption.USE_ID) && content.id == id)
                return true;

            return options.contains(SearchOption.IGNORE_CASE) ? content.name.equalsIgnoreCase(input) : content.name.equals(input);
        });
    }
    ...
    public static boolean deepEquals(String name, String input, EnumSet<SearchOption> options) {
